...
class DetailController extends Controller
{
    ...
    {
        $detail = DetailModel::select('detail_id', 'fk_penjualan_id', 'fk_barang_id', 'harga', 'jumlah')
            ->with('barang')
            ->with('penjualan');

        if ($request->fk_barang_id) {
            $detail->where('fk_barang_id', $request->fk_barang_id);
        }
    ...
    public function store_ajax(Request $request)
    {
        ...
        if ($request->ajax() || $request->wantsJson()) {
            $rules = [
                'fk_penjualan_id'  => 'required|integer',
            ...
            if (count($data) > 1) { // jika data lebih dari 1 baris
                foreach ($data as $baris => $value) {
                    if ($baris > 1) { // baris ke 1 adalah header, maka lewati
                        $insert[] = [
                            'fk_penjualan_id'  => $value['A'],
                            'fk_barang_id'     => $value['B'],
                            'harga'         => $value['C'],
                            'jumlah'        => $value['D'],
                            'created_at' => now(),
                        ];
